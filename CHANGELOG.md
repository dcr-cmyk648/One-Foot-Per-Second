...
- Rebuilt Grow/Build and pitch acquisition as deterministic run drafts. Every clear queues a level-scaled perk; sub-era finales guarantee Rare-or-better choices and a separate learn-or-level Pitch draft; boss pitches and prestige-licensed boss perks are stronger. Saved offers cannot be rerolled, skipped auto-advance rewards queue safely, and eldritch corruption can trade a random penalty for a 2–3× positive effect.
- Made Mastery outcome-aware and strikeout-gated. Called Strikes teach the most, lesser contact teaches a little, Home Runs and Grand Slams teach nothing, and a strikeout grants a completion bonus. A full bar now means “ready for the final K,” not an automatic clear; offline play cannot bypass sticky bosses. Overmastery renders repeated greener laps and keeps logarithmic farming value.
- Re-anchored the complete physical campaign: the opening one-foot-per-second ball still takes three seconds to cross three feet, human baseball reaches 115 mph, alien baseball approaches Mach 5,000 around Olympus Mound, and the final defense approaches 5,000c from Earth to Pluto. Every projectile preserves its own release speed, drag, plate speed, duration, path, source, and target generation.
- Replaced jump-ahead tapping with a shared 1.5-second rolling Tap Rate. Manual and automatic taps smoothly accelerate recovery, lineup motion, and the active ball; long waits receive more help, rapid bursts diminish, and input adds a small amount of renamed **Determination**. Added a multi-lap circular tap meter and live auto-tapper pulses.
- Added authoritative overlapping volleys. Every ball resolves independently while sharing its volley’s pitch, reciprocal ball-versus-bat coverage changes contact odds, extra arms add their own capacity, clones use separate spatial-coverage/catch/tier checks, and abandoned volleys lose targeting, veer away, and fade instead of hitting a replacement batter.
- Reworked equipment into build pieces with one to three affixes, rarity-scaled magnitude, positive/negative tradeoffs across every run stat, and one-stat Relics with eldritch slot expansion. Drops still copy a visible enemy slot/rarity, never auto-equip, preserve stars/equipped items, and convert overflow into Scrap.
- Added a persistent spoiler-gated Story journal, first-run and prestige-replay variants, broader composable names, 130 permanent +0.5% XP achievements, saved reverse ordering, and post-God procedural Extra Innings with repeatable run rewards.
- Rebuilt BODY as **RUN / DNA / ARCANA / DIVINE**, made PITCH a read-only Arsenal, added exact x1/x10/x100 Training batches, shortened passive cards, removed duplicated prices, and retained desktop-hover/phone-hold inspection parity. Future layers remain absent from interface copy until discovered.
- Retuned the first lifetime to reach human first contact in roughly nine to eleven hours under aggressive no-loot policies, still at the 115 mph physical cap. Post-human velocity now requires several meaningful prestige lives; the finite DNA and Arcana speed tracks extend far enough to earn their advertised Mach-5,000 and 5,000c licenses without impossible asymptotic Training bills. The conservative full-ladder audit reaches Octathulhu at 5,000c in about 70 simulated days and now holds the final reality for its witnessed winning K instead of prestiging one decision too early.
- Advanced saves to schema 28. Old campaign positions map by authored league position; legacy ages and pitches become equivalent deterministic run state; prestige, gear, achievements, story, active volleys, choices, lifetime history, mirrored browser generations, and update checkpoints remain preserved. Browser, PWA, native packages, Pages, and Sites continue to share one gameplay source.

## 0.16.6 — The save file remembers the family tree

- Added prestige-aware summaries to the title-screen Resume picker and every in-game manual save slot. Once a layer is revealed, each save now shows its current DNA or Arcana balance beside its lifetime-earned total; divine saves also show Blessings and Halos.
- Derived every prestige line from that slot's own save data rather than the currently loaded run, so a newly reborn toddler is visibly different from an old pre-prestige save. Legacy DNA and rebirth fields remain recognized.
- Kept unrevealed layers completely absent from early-game rows and retained compact large-number formatting. Save schema remains 25, so all v0.16.5 saves load directly.
...
- Reworked Frustration from an elapsed-time clock into an outcome-severity score: Grand Slam +12, Home Run +8, Triple +5, Double +3, Single +1, Ball +0.20, Foul +0.10, and Strike +0. Each resolved volley counts once regardless of projectile count; a completed strikeout resets the score. Four points grant the first +0.08 logarithmic quality step, with uncapped diminishing returns shown in the compact footer meter.
...
- Hardened update-safe saving with schema v17 migration, a validated pending write, a rotating previous-generation backup, preservation of unreadable data, refusal by older cached builds to overwrite newer schemas, and browser recovery that compares both a primary and rollback mirror before autosaving. If every automatic generation is unreadable, writes stay locked until LOAD or an explicitly typed RESET resolves it.
...
- Added persistent lifetime counters and peak records for active field taps, saved hits, pitch speed, mound distance, and loot rarity. Clean-run eligibility migrates from the version-16 foundation, while current saves silently backfill only achievements their history can prove.
