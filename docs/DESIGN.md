...
Clicking or tapping unobstructed field space advances whichever visible foreground timer is active: wind-up recovery, immutable ball flight, or batter lineup. The opening amount is 5% of that phase's initial duration; Field Hustle reaches 8%. A phase accepts at most 50% tap-supplied progress, so active play helps but never erases the idle half. The exact input point produces a short cyan expanding ring and percentage cue. Flight taps advance the existing projectile clock without changing its sampled speed, path, source, or outcome.
