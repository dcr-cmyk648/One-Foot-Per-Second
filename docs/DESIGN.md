...
True flight time is `release distance / exact release speed`. Visual flight preserves the literal three-second opening and logarithmically compresses extreme durations to a readable ceiling; Stats retains the true physical value. Previous/next batter controls are always available within unlocked bounds. Selecting a new target sets that opponent's range for the next release and makes the same unresolved pitch meet the new batter, while the released ball preserves its own original distance and remaining flight time.
