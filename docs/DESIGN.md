...
Save version 17 persists the explicit pitch phase, hidden pending outcome, selected pitch ID, exact sampled speed, release distance, original and remaining duration, generated batter identity/loadout, live Strike and Ball counts, nine additive Training axes, forty-two Facilities, cooldown, eight result totals, strikeouts, currencies, upgrades, mastery, the current outcome-weighted Frustration score, automation, loot, favorites, equipment, Scrap, pity, achievement IDs and trigger history, No Hitter attempt eligibility, peak speed/range/rarity records, catalog filter preferences, and lifetime statistics. Version-16 drought seconds migrate to the mathematically identical point on the new logarithmic curve. Older saves silently backfill only achievements that their persisted history proves, without emitting an unlock-toast backlog. Version-13 saves still default Scrap, Field Hustle, and both newer genetic ranks to zero while retaining every earlier migration.

Every automatic save is first written and validated as a pending generation; the preceding valid primary becomes a backup before replacement. Loading selects the newest valid generation, preserves an unreadable primary separately, and refuses to let an older cached executable overwrite a newer schema. If nothing valid remains, automatic writes lock instead of silently replacing the evidence; LOAD or an explicit typed RESET is required. The product rename retains the original custom application-data directory and save filename for continuity. Web autosaves additionally rotate synchronous primary and rollback localStorage mirrors, compare monotonic lifetime progress during startup, require a review warning and delayed filesystem flush for updates, and expose three phone manual slots without replacing portable Export/Load.
