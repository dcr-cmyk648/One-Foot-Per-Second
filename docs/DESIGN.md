...
The fresh in-game interface exposes ordinary XP, the human opponent ladder, Training, Pitch, Ball, Facility, **BODY**, Gear, Status, Help, six equipment squares, Saves, and the permanent-progress reset. Status shows all 15 effective trained stats and itemizes only upgrades already owned or revealed. BODY initially contains ordinary biological ages plus unlocked physical-development choices; its genetic, eldritch, and divine sections do not exist visibly until their story boundaries. The seven-slot Relic remains anonymous until post-human play. DNA, mutations, automation, extra bodies, and alien catalogs appear only after the portal stranger's offer. Arcana, magic, clones, reality statistics, and eldritch catalogs appear only after N'Kthra's offer. Divine controls and completion statistics appear only after the first cosmic victory.

Within a revealed story tier, its full catalog is visible and every tab is ordered by unlock level, then cost. A locked entry shows only its unmet requirements; it does not reveal the effect. Most gates are campaign levels, while selected Facility gates ask for a measured speed, a level-assigned range, or a Strikeout achievement. Descriptions state concrete arithmetic such as `Base speed +0.75 ft/s`, `Quality ×1.08`, or `Lineup time ×0.90`. Unlocked but unaffordable cards keep the same readable color as affordable cards; only the XP-price action dims. Maximum-rank and body-cap language appears only when reached. Attempting to train beyond a body limit opens a contextual hint rather than spoiling genetics.
...
The Achievements tab always exposes a 109-slot total. Each completion permanently adds one additive percentage point to XP income, so the catalog ranges from `×1.00` to `×2.09`; achievements survive genetic, eldritch, and God Prestige resets. Cards use passive text as their drag/wheel surface and delimit Details as their only inspection action. A saved Hide Achieved filter removes only completed cards. Future subjects appear only as `HIDDEN ACHIEVEMENT`. Multiple simultaneous completions enter a queued toast with both name and completed condition. `Wait… That’s Illegal` records the first learned post-human pitch. The fully secret No Hitter challenge begins with each post-victory God Prestige and is spoiled by any fair-hit outcome even when late-game defenses preserve the count.
...
Ordinary Training offers one repeatable purchase for every base stat: Speed, Quality, Field Tap, Recovery, Offline XP, Distance Control, Lineup time, fair-hit Delay, Pitch Calling, Payload, Mastery, Drag, Strikeout XP, Loot chance, and Frustration response. Speed adds `0.75 ft/s` per rank; Command adds only `0.018` quality, and velocity supplies a separate logarithmic quality contribution. Each visible card calculates the exact next-rank delta against the current effective stat, while hover or a stationary phone hold adds the full formula. Status and the phone Upgrades header show all 15 values. The compact Live Throw Profile retains only the last immutable throw: pitch, release speed, plate speed, drag loss, travel time, quality, and range. Thirty-one pitches, 26 ball shells, 84 one-time Facilities/projects, 12 age stages, and 12 BODY modifiers provide the larger multiplicative layer. The post-human pitch catalog deliberately includes extreme tradeoffs such as a very slow, high-quality Gazorpian Strudelball and a very fast, inaccurate Andromedan Knucklebomb.

Clicking or tapping unobstructed field space advances whichever visible foreground timer is active: wind-up recovery, immutable ball flight, or the complete next-batter handoff. The opening amount is 1.7% of that phase's initial duration; repeatable Field Hustle approaches 4% with diminishing returns. A phase accepts at most 50% tap-supplied progress, so active play helps but never erases the idle half. The exact input point produces a short cyan expanding ring and percentage cue. Flight taps advance the existing projectile clock without changing its sampled speed, path, source, or outcome; lineup taps advance the authoritative cooldown and the visible on-deck meter together.
...
Bambino Rex requires the first body's 115 mph limit, modestly beyond the fastest verified real pitch while leaving obvious room for the next league. Clearing human baseball reveals Xylophax, Genetic Commissioner, whose first witnessed minute is guaranteed Grand Slams regardless of mastery, Frustration, gear, or stats. The clock does not advance offline. At one minute a small red HELP button appears without interrupting play; only clicking it reveals a portal stranger who says, “Come with me if you want to… be really good at baseball,” and grants persistent access to prenatal genetic engineering and a Time Machine.
...
Save version 22 persists body age, BODY modifiers, and the toddler-clear proof in addition to the explicit pitch phase, hidden pending outcome, selected pitch ID, exact sampled release/plate speed and drag, release distance, original and remaining duration, generated batter identity/loadout, live Strike and Ball counts, 15 additive Training axes, 84 Facilities, cooldown, eight result totals, strikeouts, currencies, upgrades, mastery, outcome-weighted Frustration, per-stat/per-catalog automation choices, human and alien Auto-advance capacity, loot with its defeated-batter source, favorites, equipment, Scrap, pity, 109 achievement IDs and trigger history, filters, No Hitter eligibility, peak records, and lifetime statistics. Version 22 maps the former six-stage body safely into the 12-stage ladder and moves owned human chemistry out of retired Facilities into matching BODY modifiers. Earlier distance, age, Frustration, automation, loot, and Training migrations remain intact; older saves silently backfill only achievements their persisted history proves, without an unlock-toast backlog.

Every automatic save is first written and validated as a pending generation; the preceding valid primary becomes a backup before replacement. Loading selects the newest valid generation, preserves an unreadable primary separately, and refuses to let an older cached executable overwrite a newer schema. If nothing valid remains, automatic writes lock instead of silently replacing the evidence; IMPORT or an explicit typed RESET is required. The product rename retains the original custom application-data directory and save filename for continuity. All platforms expose an autosave plus three device-local manual slots from both the title Resume picker and in-game SAVES overlay. Web builds additionally mirror those slots and rotating primary/rollback generations in localStorage, compare monotonic lifetime progress during startup, flush on background lifecycle events, and use a bounded Export/Later/Update review plus an activation watchdog. Portable Export/Import remains the cross-device transfer path; mobile Files providers such as Google Drive can participate through the operating-system picker without granting the game account access.
