...
The Achievements tab always exposes a 108-slot total. Each completion permanently adds one additive percentage point to XP income, so the catalog ranges from `×1.00` to `×2.08`; achievements survive genetic, eldritch, and God Prestige resets. Cards use passive text as their drag/wheel surface and delimit Details as their only inspection action, following the default phone-menu rule that scrolling content is not itself a button. A saved Hide Achieved filter removes only completed cards. Human, post-human, eldritch, and divine milestones share the catalog, but future information obeys a stricter disclosure rule than ordinary upgrade locks. Before its subject has been encountered, a card says only `HIDDEN ACHIEVEMENT`: it does not show its name, condition, progress, description, tooltip detail, or unrevealed tier heading. Completing a secret reveals it immediately. Multiple simultaneous completions enter a queued toast rather than overwriting one another. The fully secret No Hitter challenge begins with each post-victory God Prestige, survives lower resets, and is spoiled by any fair-hit outcome even when late-game defenses preserve the count. Repeat universes remember the two prestige offers and pause at their exhibition gates, making a genuinely clean campaign possible without erasing the original scripted introductions.

Ordinary growth is a separate sequential XP channel: Toddler, Little Kid, Big Kid, Teenager, Young Adult, and Regular Ol’ Guy. The toddler starts with a modest relative speed, quality, and recovery penalty; the immediate 3-XP Little Kid purchase remains useful without abruptly skipping the early cadence. Full adulthood is approximately `×1.38` speed, `+0.15` quality, and `×1.16` recovery over the toddler. Each purchased age also establishes a larger visual-size floor. Training, payload, mutations, arms, clones, and time layers add further saturating visual strength, approaching but never exceeding twice the original intrinsic pitcher size. Every time-travel reset begins at Toddler again. A secret achievement records clearing all human opponents without buying any age.

Ordinary Training offers one additive purchase per base stat, unlocked gradually in this order: Speed, Quality, active Field Tap, Recovery, Offline XP, Distance Control, base Lineup time, fair-hit Delay, and Pitch Calling. Speed adds `0.50 ft/s` per rank; Command adds only `0.04` quality, and velocity itself supplies a separate logarithmic quality contribution. Status shows all nine effective values. The compact field profile is deliberately narrower and contains only the current immutable throw: pitch, release speed, plate speed, drag loss, travel time, quality, and range. The phone Upgrades overlay pins current effective values above its independently scrolling tabs, and only its explicit touch-sized previous/next buttons around a large current-section card navigate tabs; native overflow arrows are disabled. Twenty-two pitches, twenty-six ball shells, and eighty-six one-time Facilities/projects provide the separate multiplicative layer, so a new pitch laboratory or questionable intervention is a larger event than pumping another Training rank. The expensive human-project lane keeps meaningful targets available from hundreds of XP through trillions. There are no same-operation quality buttons with cosmetic names.
...
Fifteen ranges run from 3 feet through 6, 12, 20, 30, 44, 60.5, 90, and 300 feet, then one mile, low orbit, Earth-to-Moon, one AU, one light-year, and 100,000 light-years. Each range is assigned by opponent level at the same authored unlock threshold; it is not a separate player-selected optimization. Farther stages raise XP and threat. Long-Toss Mechanics multiplicatively reduces only the threat penalty.
...
Save version 21 persists body age and the toddler-clear proof in addition to the explicit pitch phase, hidden pending outcome, selected pitch ID, exact sampled release/plate speed and drag, release distance, original and remaining duration, generated batter identity/loadout, live Strike and Ball counts, nine additive Training axes, eighty-six Facilities, cooldown, eight result totals, strikeouts, currencies, upgrades, mastery, outcome-weighted Frustration, per-stat/per-catalog automation choices, human and alien Auto-advance capacity, loot, favorites, equipment, Scrap, pity, 108 achievement IDs and trigger history, filters, No Hitter eligibility, peak records, and lifetime statistics. Version-20's former single all-human Auto-advance purchase migrates to all 29 human destination licenses. Loading an older manual distance safely replaces it with the selected opponent's authored range while preserving any live projectile's immutable release snapshot. Earlier age, Frustration, automation, loot, and Training migrations remain intact; older saves silently backfill only achievements their persisted history proves, without an unlock-toast backlog.
